    ...
    <Resizable
      ...
      ref={(ref) => {
        if (ref) {
          ...
          // Craft connectors must attach to the underlying resizable DOM node, not the React wrapper.
          connect(resizable.current.resizable);
        }
      }}
      size={internalDimensions}
      onResizeStart={(e) => {
        // Convert percentage sizing into px before drag begins because re-resizable mutates pixel values.
